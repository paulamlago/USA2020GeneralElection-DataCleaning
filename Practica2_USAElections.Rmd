...
Como se puede observar, el modelo que mejor se ha ajustado ha sido el tercero puesto que su coeficiente de determinación ha sido el más elevado. Ahora, empleando este modelo, podemos proceder a realizar un prediccion de participación en las elecciones en Minnestoa. 
```{r}
...
Definimos las funciones calc_cmatrix y get_precision para calcular la matriz de confusión y calcular la precisión del modelo
...
get_precision(table_results)
```
El modelo que mejor precisión ha presentado ha sido el primero, por lo que se va a aplicar una predicción sobre los datos de test con dicho modelo para ver si es capaz de predecir que estado ganó en Minnesota.
...
Como se puede observar, el algoritmo ha sido capaz de predecir correctamente el estado ganador en Minnesota.


# 5. Representación de los resultados a partir de tablas y gráficas

Para la visualización de los datos se utlizará también el lenguaje Python, dado que ofrece la representación de mapas. Este código ha sido ejecutado en un [Jupyter Notebook](https://github.com/paulamlago/USA2020GeneralElection-DataCleaning/blob/master/Visualisations.ipynb) y los resultados pueden verse a continuación. 
```{python eval = FALSE}
...
Las regresiones realizadas nos han indicado cuál es la tendencia electoral en EEUU, y dejan abierta una oportunidad de estudio de estos datos en vistas a las próximas elecciones. La regresión lineal múltiple aplicada utilizando todas las variables cuantitativas ha resultado en un modelo con una confianza del 64%, mientras que la regresión logística ha resultado mucho mejor para esta tarea, ofreciendo una confianza del 74% a la hora de predecir el partido ganador en cada estado.

# 7. Código

El código se encuentra disponible en el siguiente repositorio de GitHub (https://github.com/paulamlago/USA2020GeneralElection-DataCleaning)
...
|-----------------------------|---------------------------------------|
| Investigación previa        | Abel Romero Búrdalo, Paula Muñoz Lago |
| Redacción de las respuestas | Abel Romero Búrdalo, Paula Muñoz Lago |
| Desarrollo Código           | Abel Romero Búrdalo, Paula Muñoz Lago |
