...
## -------------------------------------------------------------------------------------------------------------------------------------------------------------
usa_elections <- usa_elections[,-c(grep("Source", colnames(usa_elections)),
                                   grep("Official.Unofficial", colnames(usa_elections)))]
...
mean_total=mean(
  usa_elections$
    Total.Ballots.Counted..Estimate.
      [!is.na(usa_elections$Vote.for.Highest.Office..President.)]
  , na.rm=TRUE)

# Sacamos el procentaje de la media de votos válidos
percentage_votes= mean_president/mean_total

# Aplicamos dicho porcentage a los votos totales que no disponen dicha información 
#y guardamos los votos válidos en su correspondiente estado
# Como los votos deben de ser un numero entero se va a redondear el resultado de 
#multiplicar los votos totales por el porcentage de votos válidos
usa_elections$Vote.for.Highest.Office..President.<-
  ifelse(is.na(usa_elections$Vote.for.Highest.Office..President.),
         trunc(usa_elections$Total.Ballots.Counted..Estimate. * percentage_votes), 
         usa_elections$Vote.for.Highest.Office..President.  )
...
replace_0 <- function(column_index) {
  ...
  mean_total = mean(
    usa_elections$Total.Ballots.Counted..Estimate.[!is.na(usa_elections[,column_index])]
    )
  
  percentage_votes = mean_column/mean_total
  
  final_column <- ifelse(
    usa_elections[,column_index] == 0,
    trunc(usa_elections$Total.Ballots.Counted..Estimate. * percentage_votes), 
    usa_elections[,column_index])
...
## -------------------------------------------------------------------------------------------------------------------------------------------------------------
write.csv(usa_elections, 
          "2020 November General Election - Turnout Rates_data_clean.csv",  
          row.names = FALSE)
...
for (i in 1:ncol(usa_elections)) {
  if (i == 1) cat("Variables que no siguen una distribución normal 
                  segun el test de Anderson-Darling:\n")
...
# Predecir el turnout
turout_predicted <- predict(modelo3, lineardata)


## -------------------------------------------------------------------------------------------------------------------------------------------------------------
cat("El porcentaje de participación que hubo en minessota es: ", test$VEP.Turnout.Rate,
  "\nfrente al porcetaje predecido por nuestro modelo, que ha resultado: ", turout_predicted)


## -------------------------------------------------------------------------------------------------------------------------------------------------------------
# Generación de varios modelos
# Modelo 1
modelo1 <- glm(as.factor(winner) ~  votos + vep + probation +parole +turnout, 
               data =train,family=binomial(link=logit))
# Modelo 2
modelo2 <- glm(as.factor(winner) ~  turnout  + parole +probation, 
               data = train, family=binomial(link=logit))
# Modelo 3
modelo3 <- glm(as.factor(winner) ~  votos_validos   +probation + parole + vap , 
               data = train, family=binomial(link=logit))
# Modelo 4
modelo4 <- glm(as.factor(winner) ~  votos_validos + probation + felon+prision , 
               data = train, family=binomial(link=logit))
# Modelo 5
modelo5 <- glm(as.factor(winner) ~  turnout + votos + vep +vap+ felon, 
               data = train, family=binomial(link=logit))
...
# Generamos un vector en el que si la predicción es superior 
  # a 0.5 se clasifica como REP y en caso contrario como DEM
...
## -------------------------------------------------------------------------------------------------------------------------------------------------------------
cat("Ganador en Minnesota", levels(test$party_winner)[test$party_winner], 
    "\nGanador predecido en dicho estado por nuestro modelo: ", predicted_winner)
